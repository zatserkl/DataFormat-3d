...
public:
   ...
   Sensor(Bool_t debug=kFALSE): debug_(debug), sensorId_(0), pitch_(0.228) {
      if (!poolSensorHit_) {
   ...
   void GetHits() {
      ...
      while (it != clusters_.end()) {
         ...
         if (debug_) cout<< "Sensor::GetHits: " << sensorHit << "\tsensorId: " << sensorId_ << " layer " << layer_ << " sensor " << sensorId_ << " nfirst = " << std::setw(3) << nfirst << " nstrips = " << nstrips << " coordinate = " << CenterOfGravity(nfirst) <<endl;
      }
   }
};
...
public:
   VSensor(Bool_t debug=kFALSE): Sensor(debug) {}
...
public:
   TSensor(Bool_t debug=kFALSE): Sensor(debug) {}
   void Set_strip1_dir_info(Double_t u, Double_t strip1, Double_t dir, Int_t layer, Int_t sensor) {
...
public:
   ...
   PCTSensors(const Geometry* geometry, Bool_t debug=kFALSE): geometry_(geometry), debug_(debug)
   {
      ...
      if (debug_) {
         for (int ilayer=0; ilayer<4; ++ilayer) {
            for (int isensor=0; isensor<2; ++isensor) {
               vSensor[ilayer][isensor].debug_ = debug_;
               tSensor[ilayer][isensor].debug_ = debug_;
               tSensor[ilayer][isensor+2].debug_ = debug_;
            }
         }
      }
