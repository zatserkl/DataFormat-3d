...
TClonesArray* Reco::poolTrack_ = 0;
             ...
             )
{
   ...
   tree->SetBranchAddress("revent", &recoEvent);      // point recoEvent to the buffer
   ...
   for (int ientry=event1; ientry<=event2; ++ientry)
   {
      ...
      Reco reco(geometry, &beamSpot, pCTSensors, pCTEvent, ientry, debug);
              ...
              )
{
   ...
   PCTSensors* pCTSensors = new PCTSensors(geometry, debug);
   ...
   Reco reco(geometry, &beamSpot, pCTSensors, pCTEvent, event, debug);
   ...
   for (int itrack=0; itrack<recoEvent->track->GetLast()+1; ++itrack) {
      ...
      // cout<< "v = " << v << " t = " << t <<endl;
      //-- cout<< *superTrack <<endl;
      cout<< itrack << "\tangle = " << superTrack->angle << " distance = " << superTrack->Distance() <<endl;
      cout<< "\tsuperTrack->itrack_->vTrack_->hit1_ = " << *superTrack->itrack_->vTrack_->hit1_ << "\tsuperTrack->itrack_->vTrack_->hit2_ = " << *superTrack->itrack_->vTrack_->hit2_ <<endl;
      cout<< "\tsuperTrack->itrack_->tTrack_->hit1_ = " << *superTrack->itrack_->tTrack_->hit1_ << "\tsuperTrack->itrack_->tTrack_->hit2_ = " << *superTrack->itrack_->tTrack_->hit2_ <<endl;
      cout<< "\tsuperTrack->otrack_->vTrack_->hit1_ = " << *superTrack->otrack_->vTrack_->hit1_ << "\tsuperTrack->otrack_->vTrack_->hit2_ = " << *superTrack->otrack_->vTrack_->hit2_ <<endl;
      cout<< "\tsuperTrack->otrack_->tTrack_->hit1_ = " << *superTrack->otrack_->tTrack_->hit1_ << "\tsuperTrack->otrack_->tTrack_->hit2_ = " << *superTrack->otrack_->tTrack_->hit2_ <<endl;
   }
...
Int_t display(Int_t event, TTree* tree=0, const char* dbname="rundb-Jul2014.dat", const char* wname="event_display")
{
   if (!tree) tree = (TTree*) gDirectory->Get("t");
   if (!tree) {
      cout<< "Could not find tree t" <<endl;
...
void eloop(Int_t evtNo=0, TTree* tree=0, const char* dbname="rundb-Jul2014.dat", const char* wname="event_display")
{
   if (tree == 0) {
